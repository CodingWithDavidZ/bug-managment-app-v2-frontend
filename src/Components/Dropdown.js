...
function Dropdown({ array, label }) {
	...
	useEffect(() => {
		...
		// map thorough the array to make 2 separate arrays depending on if separated key is true or false
		optionsArray.map((option) => {
			if (option.separated === true) {
				setSeparatedArray(...{option});
			} else if (option.separated === false) {
				setUnseparatedArray(...option);
			}
		});
	}, [array, label]);

	console.log('separatedArray', separatedArray, 'unseparatedArray', unseparatedArray);
	...
	function handleChange(e) {
		if (e.target.id === 'noSelection') {
			setDropdownLabel(noSelection);
			changeVisible();
		} else {
			setDropdownLabel(e.target.id);
			changeVisible();
		}
	}
